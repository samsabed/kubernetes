...
func (s *statusManager) loadFromKubeStore() {
	iMap, err := s.kubeStore.LoadMap(BucketPodStatuses, func() interface{} { return &podStatusInfoType{} })
	if err != nil {
		glog.Warningf("unable to load data from kubestore %v", err)
	}
	...
	for k, v := range iMap {
		podStatusInfo, ok := v.(*podStatusInfoType)
		if !ok {
			glog.Warningf("loadFromKubeStore: cant convert k=%v v=%v", k, v)
			continue
		}
		s.podStatuses[types.UID(k)] = *podStatusInfo
	}
}
